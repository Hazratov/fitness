...
interface ContentTypeDialogProps {
  ...
  contentType: "mashqlar" | "taomnnoma";
  onContentTypeChange: (type: "mashqlar" | "taomnnoma") => void;
}
...
const ContentTypeDialog: React.FC<ContentTypeDialogProps> = ({ 
  ...
  const handleExerciseClick = () => {
    onContentTypeChange("mashqlar");
    onOpenChange(false);
  };
  
  const handleMealClick = () => {
    onContentTypeChange("taomnnoma");
    onOpenChange(false);
  };
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="bg-[#111827] border-[#2a3547] text-white max-w-3xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
          {/* Exercise Option */}
          <div 
            className={`bg-gradient-to-br from-[#1a2336] to-[#1e293b] p-8 rounded-xl border ${contentType === "mashqlar" ? "border-[#3b82f6]" : "border-[#2a3547]"} hover:shadow-lg hover:border-[#3b82f6]/30 transition-all cursor-pointer`}
          ...
          {/* Meal Option */}
          <div 
            className={`bg-gradient-to-br from-[#1a2336] to-[#1e293b] p-8 rounded-xl border ${contentType === "taomnnoma" ? "border-[#10b981]" : "border-[#2a3547]"} hover:shadow-lg hover:border-[#10b981]/30 transition-all cursor-pointer`}
